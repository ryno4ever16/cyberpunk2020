...
/**
 * QA §16 — GM/player apply-damage routing.
 *
 * A player cannot write to an actor they don't own. So when a player applies
 * damage to an enemy, the damage dialog emits a socket request and the GM (the
 * active one) applies it with GM permissions, then acks the requester.
 *
 * This proves the whole round-trip:
 *   1. The player genuinely CANNOT modify the unowned NPC directly (permission).
 *   2. The player CAN modify their own owned character directly (contrast).
 *   3. The player's socket request results in the NPC taking damage (the GM
 *      applied it) and the player receiving a "damageApplied" ack.
 *   4. Exactly the requested amount is applied — once, not multiplied by GM count
 *      (the socket relay is already activeGM-gated).
 */
