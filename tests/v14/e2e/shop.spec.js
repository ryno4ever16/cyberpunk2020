...
/**
 * Rig-portable port of tests/e2e/shop.spec.js — multi-account; logins swapped to loginRig/loginRigAs + ensureRigUsers.
 *
...
test.afterAll(async ({ browser }) => {
  ...
  try { await loginRig(p); await cleanupTestData(p); await resetShops(p); } catch {}
  await ctx.close();
});
...
test("player buys from a published ShopDef; limited stock depletes via GM relay", async ({ browser }) => {
  ...
  try {
    ...
    await loginRigAs(pl, "Test User 1");
