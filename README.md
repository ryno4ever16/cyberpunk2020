...
* **Upgrading re-applies stock cyberware templates.** When a world migrates to a newer version, each installed cyberware is re-synced to its compendium template. Placement (limb/side), Humanity loss, cost, weight, equipped/active state, module links, and a *custom (renamed) name* are preserved; other manual edits to a *recognized stock* cyberware — notably its **notes** and any hand-edited mechanical fields — revert to the template. To keep custom edits across upgrades, **rename the item** or use a homebrew / non-canon cyberware entry (those are never overwritten).
...
* **Maximum Metal — some optional rules are not modelled yet.** The errata "armour damage via penetration" / high‑density‑AP rules (p.107) and the optional crew‑morale rule are not automated; resolve them manually if you use them.
